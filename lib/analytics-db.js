...
export const trackImageGeneration = async (userId, style, prompt = null) => {
  try {
    ...
    // Create image generation record for EACH image generated
    // If user generates 4 images, create 4 records
    for (let i = 0; i < imageCount; i++) {
      await prisma.imageGeneration.create({
        data: {
          userId,
          style,
          prompt: i === 0 ? prompt : null // Only store prompt on first record
        }
      });
    }

    // Update user's total generated with actual count
    await prisma.user.update({
      where: { userId },
      data: {
        totalGenerated: {
...
export const getAnalyticsSummary = async () => {
  try {
    ...
    // Process style breakdown - merge similar styles
    const styleBreakdownObj = {};
    styleBreakdown.forEach(item => {
      // Clean up style name - remove " - สไตล์ X" suffix
      let cleanStyle = item.style;
      if (cleanStyle) {
        ...
        // Aggregate counts for same style
        if (styleBreakdownObj[cleanStyle]) {
          styleBreakdownObj[cleanStyle] += item._count;
        } else {
          styleBreakdownObj[cleanStyle] = item._count;
        }
      }
