...
const useStore = create(
  persist(
    (set, get) => ({
      ...
      // Video Generation State (to lock mode switching)
      isGeneratingVideo: false,
      setIsGeneratingVideo: (generating) => set({ isGeneratingVideo: generating }),
      ...
      // Video Ads Modal State (to open from History page)
      showVideoAdsModal: false,
      videoAdsPreloadedImage: null,
      openVideoAdsModal: (image = null) => set({ showVideoAdsModal: true, videoAdsPreloadedImage: image }),
      closeVideoAdsModal: () => set({ showVideoAdsModal: false, videoAdsPreloadedImage: null }),
      ...
      // Add video to history (similar to image history)
      addVideoToHistory: async (video) => {
        try {
          const videoHistoryItem = {
            ...video,
            id: video.id || `video_${Date.now()}_${Math.random()}`,
            timestamp: video.timestamp || new Date().toISOString(),
            type: 'video' // Mark as video type
          }

          // Save to IndexedDB
          await saveToHistory(videoHistoryItem)
          ...
          console.log('Added video to history')
        } catch (error) {
          console.error('Error adding video to history:', error)
        }
      },

      // User System - NOT persisted, loaded from localStorage directly
      userId: null,
      setUserId: (userId) => set({ userId }),

      // Credits System - NOT persisted, loaded from localStorage directly
      userCredits: 0, // Default credits - start at 0
      setUserCredits: (credits) => set({ userCredits: credits }),
      useCredits: async (amount = 1) => {
        const state = get()
        const userId = state.userId || localStorage.getItem('nano_user_id')
        ...
        try {
          ...
          if (data.success) {
            ...
            // Update localStorage
            localStorage.setItem(`nano_credits_${userId}`, data.credits.toString())
            return true
          } else {
            console.error('Failed to use credits:', data.message)
            return false
          }
        } catch (error) {
      ...
      addCredits: (amount) => set((state) => {
        ...
        // Get userId from localStorage if not in state
        const userId = state.userId || localStorage.getItem('nano_user_id')
        if (userId) {
      ...
      // Refund credits with database sync
      refundCredits: async (amount, reason = 'API Error') => {
        const state = get()
        const userId = state.userId || localStorage.getItem('nano_user_id')

        if (!userId) {
          console.error('No user ID found for refund')
          return false
        }
        ...
        try {
          ...
          if (data.success) {
            set({ userCredits: data.newBalance })
            localStorage.setItem(`nano_credits_${userId}`, data.newBalance.toString())
            console.log(`✅ Refunded ${amount} credits successfully. New balance: ${data.newBalance}`)
            return true
          } else {
            console.error('Failed to refund credits:', data.message)
            return false
          }
        } catch (error) {
      ...
      loadUserCredits: async (userId) => {
        if (!userId) {
          set({ userCredits: 0, userId: null })
          return 0
        }

        try {
          // Get credits from database via API
          const response = await fetch(`/api/credits?userId=${userId}`)
          const data = await response.json()

          if (data.success) {
            set({ userCredits: data.credits, userId })
            ...
            const storedCredits = localStorage.getItem(`nano_credits_${userId}`)
            const credits = storedCredits ? parseInt(storedCredits) || 0 : 0
